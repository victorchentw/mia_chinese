...
>
> **目前狀態（v0.1.19 / versionCode 20）**：Phase 0A、Phase 1、Phase 2 與 MP4 播放核心已完成；API 28 baseline 的 YouTube WebView 仍未通過通用 Go/No-Go，YouTube 播放方式可在設定頁選擇 WebView 或 SmartTube／系統外部播放器。PDF 已加入 TV WebView 實驗頁；手機 QR code 會即時取得 Notion 暫時簽名下載網址，避免直接使用失效的 S3 source URL。真實目標 TV、媒體搬遷與家長功能仍是後續工作。
...
- repository 已建立 Android TV 單 module project；目前工作樹以 `v0.1.19` 進行 release hardening。
